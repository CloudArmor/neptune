...
from tests.e2e.utils import Environment, a_project_name, initialize_container

from neptune import Project, init_model_version
    ...
    def test_standard_scenario(self, environment: Environment):
        ...
        self._assure_presence_and_role(
            ...
            member_list=get_workspace_member_list(
                workspace=environment.workspace, api_token=environment.admin_token
            ),
        )
        self._assure_presence_and_role(
            username=environment.user,
        ...
        assert created_project_identifier not in get_project_list(
            api_token=environment.user_token
        )
...
class TestTrashObjects(BaseE2ETest):
    ...
    def test_trash_runs_and_models(self, project, environment):
        ...
        model1_id = initialize_container(
            ContainerType.MODEL, project=environment.project
        )["sys/id"].fetch()
        model2_id = initialize_container(
            ContainerType.MODEL, project=environment.project
        )["sys/id"].fetch()
    ...
    def test_trash_model_version(self, environment):
        ...
        model_version1_id = init_model_version(
            model=model_id, project=environment.project
        )["sys/id"].fetch()
        model_version2_id = init_model_version(
            model=model_id, project=environment.project
        )["sys/id"].fetch()
        self.wait_for_containers(
            [model_version1_id, model_version2_id], model.fetch_model_versions_table
        )
    ...
    def wait_for_containers(
        self, ids: List[str], container_provider: Callable[[], Table]
    ):
        fetched_entries = container_provider().to_pandas()
        actual_ids = (
            fetched_entries["sys/id"].tolist() if len(fetched_entries) > 0 else []
        )
...
class TestDeleteFromTrash:
    def test_delete_from_trash(self, environment):
        ...
        with initialize_container(
            ContainerType.PROJECT, project=environment.project
        ) as project:
    ...
    def wait_for_containers_in_trash(
        self, expected_run_count: int, expected_model_count: int, project: Project
    ):
