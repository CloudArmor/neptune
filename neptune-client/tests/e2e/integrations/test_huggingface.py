...
from tests.e2e.utils import catch_time, modified_environ, tmp_context
...
class TestHuggingFace(BaseE2ETest):
    ...
    def _test_with_run_initialization(self, environment, *, pre, post):
        with init_run(
            project=environment.project, api_token=environment.user_token
        ) as run:
    ...
    def _test_monitoring_cpu_present(
        self, environment, table_entry: TableEntry
    ) -> bool:
        with init_run(
    ...
    def test_run_access_methods(self, environment):
        ...
        trainer = transformers.Trainer(
            **self._trainer_default_attributes, callbacks=[callback]
        )

        assert (
            callback.run.get_url()
            == transformers.integrations.NeptuneCallback.get_run(trainer).get_url()
        )
    ...
    def test_log_parameters_with_base_namespace(self, environment):
        ...
        def run_test(run):
            callback = transformers.integrations.NeptuneCallback(
                run=run, base_namespace=base_namespace
            )
            trainer = transformers.Trainer(
                **self._trainer_default_attributes, callbacks=[callback]
            )
        ...
        def assert_metadata_structure(run):
            ...
            assert (
                run[f"{base_namespace}/trainer_parameters/num_train_epochs"].fetch()
                == 500
            )
    ...
    def test_log_with_custom_base_namespace(self, environment):
        ...
        def assert_metadata_structure(run):
            ...
            assert (
                run[f"{base_namespace}/train/after_training_metric"].fetch_last()
                == 2501
            )
    ...
    def test_non_monitoring_runs_creation_with_initial_run(
        self, environment, project, common_tag
    ):
        # given
    ...
    def test_hyperparameter_optimization(self, environment, project, common_tag):
        ...
        trainer = transformers.Trainer(
            **trainer_config, model_init=model_init, callbacks=[callback]
        )
    ...
    def test_usages(self):
        ...
        trainer_args["args"] = transformers.TrainingArguments(
            "model", report_to=["all"]
        )
        ...
        trainer = transformers.Trainer(
            **self._trainer_default_attributes,
            callbacks=[transformers.integrations.NeptuneCallback],
        )
    ...
    def _test_checkpoints_creation(
        ...
        def run_test(run):
            callback = transformers.integrations.NeptuneCallback(
                run=run, log_checkpoints=log_checkpoints
            )
        ...
        def assert_metadata_structure(run):
            ...
            with tmp_context():
                run[f"finetuning/checkpoints/{checkpoints_key}"].download(
                    "checkpoints.zip"
                )

                with ZipFile("checkpoints.zip") as handler:
                    subdirectories = set(
                        [os.path.dirname(x) for x in handler.namelist()]
                    )
