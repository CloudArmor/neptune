...
from tests.e2e.utils import generate_image, image_to_png, tmp_context

from neptune.metadata_containers import MetadataContainer
from neptune.types import FileSeries, FloatSeries, StringSeries
...
class TestSeries(BaseE2ETest):
    ...
    def test_float_series_type_assign(self, container: MetadataContainer):
        with self.run_then_assert(container, "floats") as (
            namespace,
            values,
            steps,
            timestamps,
            ...
            namespace.assign(
                FloatSeries(values=values, steps=steps, timestamps=timestamps)
            )

    @pytest.mark.parametrize("container", AVAILABLE_CONTAINERS, indirect=True)
    def test_string_series_type_assign(self, container: MetadataContainer):
        with self.run_then_assert(container, "strings") as (
            namespace,
            values,
            steps,
            timestamps,
            ...
            namespace.assign(
                StringSeries(values=values, steps=steps, timestamps=timestamps)
            )

    @pytest.mark.parametrize("container", AVAILABLE_CONTAINERS, indirect=True)
    def test_file_series_type_assign(self, container: MetadataContainer):
        with self.run_then_assert(container, "files") as (
            namespace,
            values,
            steps,
            timestamps,
            ...
            namespace.assign(
                FileSeries(values=values, steps=steps, timestamps=timestamps)
            )
            ...
            assert (
                list(
                    map(
                        ...
                        container[key].fetch_values()["timestamp"],
                    )
                )
                == timestamps
            )
