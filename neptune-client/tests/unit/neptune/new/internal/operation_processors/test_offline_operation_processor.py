...
@patch(
    "neptune.internal.operation_processors.offline_operation_processor.OperationStorage"
)
...
def test_setup(_, __, ___, ____, mkdir_mock, random_choice_mock):
    ...
    processor = OfflineOperationProcessor(
        container_id=container_id, container_type=container_type, lock=MagicMock()
    )
