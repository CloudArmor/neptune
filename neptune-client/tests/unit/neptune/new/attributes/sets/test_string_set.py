...
from neptune.attributes.sets.string_set import StringSet, StringSetVal
...
class TestStringSet(TestAttributeBase):
    ...
    def test_assign(self, get_operation_processor):
        ...
        with self._exp() as exp:
            ...
            processor.enqueue_operation.assert_has_calls(
                [
                    call(ClearStringSet(path), wait=False),
                    call(AddStrings(path, expected), wait=wait),
                ]
            )
    ...
    def test_add(self, get_operation_processor):
        ...
        with self._exp() as exp:
            ...
            processor.enqueue_operation.assert_called_with(
                AddStrings(path, {"a", "bb", "ccc"}), wait=wait
            )
    ...
    def test_remove(self, get_operation_processor):
        ...
        with self._exp() as exp:
            ...
            processor.enqueue_operation.assert_called_with(
                RemoveStrings(path, {"a", "bb", "ccc"}), wait=wait
            )
    ...
    def test_remove_single_value(self, get_operation_processor):
        ...
        with self._exp() as exp:
            ...
            processor.enqueue_operation.assert_called_with(
                RemoveStrings(path, {"bb"}), wait=wait
            )
