        ...
        expected_result = pd.DataFrame(
            {
                ...
                "epoch_channel": [np.nan, np.nan, np.nan, 0.35, 5.4, 0.9, np.nan],
            },
            dtype=float,
        )
        expected_result = sort_df_by_columns(expected_result)
...
class TestMergeDataFrames(unittest.TestCase):
    def setUp(self):
        ...
        self.df1 = pd.DataFrame({"x": list(range(10)), "y1": random_df1})

        random_df2 = np.random.random(3).tolist()
        self.df2 = pd.DataFrame({"x": list(range(3)), "y2": random_df2})

        random_df3 = np.random.random(6).tolist()
        self.df3 = pd.DataFrame({"x": list(range(6)), "y3": random_df3})

        df_merged_outer = pd.DataFrame(
            {
                "x": list(range(10)),
                "y1": random_df1,
