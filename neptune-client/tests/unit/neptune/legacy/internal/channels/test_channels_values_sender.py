...
class TestChannelsValuesSender(unittest.TestCase):
    ...
    _NUMERIC_CHANNEL.update(name="1numeric", channelType=ChannelType.NUMERIC.value)

    _TEXT_CHANNEL = a_channel()
    _TEXT_CHANNEL.update(name="2text", channelType=ChannelType.TEXT.value)

    _IMAGE_CHANNEL = a_channel()
    _IMAGE_CHANNEL.update(name="3image", channelType=ChannelType.IMAGE.value)
