...
from typing import TYPE_CHECKING, Callable, Dict, Iterable, List, NamedTuple, Tuple

from neptune.cli.containers import AsyncContainer, ExecutionDirectory, OfflineContainer
...
def collect_containers(*, path: Path, backend: "NeptuneBackend") -> CollectedContainers:
    ...
    return CollectedContainers(
        ...
        unsynced_containers=[
            x for x in async_containers if not x.synced and x.found is True
        ],
        not_found_containers=[x for x in async_containers if x.found is False],
    )


def collect_async_containers(
    *, path: Path, backend: "NeptuneBackend"
) -> Iterable[AsyncContainer]:
    container_to_execution_dirs = collect_by_container(
        base_path=path / ASYNC_DIRECTORY, detect_by=detect_async_dir
    )

    for (
        container_type,
        container_id,
    ), execution_dirs in container_to_execution_dirs.items():
        experiment = get_metadata_container(
            backend=backend, container_type=container_type, container_id=container_id
        )
...
def collect_offline_containers(*, path: Path) -> Iterable[OfflineContainer]:
    container_to_execution_dirs = collect_by_container(
        base_path=path / OFFLINE_DIRECTORY, detect_by=detect_offline_dir
    )
...
def collect_child_directories(
    base_path: Path, structure_version: StructureVersion
) -> List[Path]:
    if structure_version in {
...
def collect_by_container(
    *,
    base_path: Path,
    detect_by: Callable[[str], Tuple[ContainerType, UniqueId, StructureVersion]],
) -> Dict[Tuple[ContainerType, UniqueId], List["ExecutionDirectory"]]:
    container_to_execution_dirs: Dict[
        Tuple[ContainerType, UniqueId], List["ExecutionDirectory"]
    ] = defaultdict(list)
    ...
    for child_path in base_path.iterdir():
        ...
        for execution_dir in execution_dirs:
            parent = (
                ...
                if structure_version == StructureVersion.CHILD_EXECUTION_DIRECTORIES
                else None
            )
