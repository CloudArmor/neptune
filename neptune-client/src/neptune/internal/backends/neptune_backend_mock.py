...
from neptune.internal.id_formats import QualifiedName, SysId, UniqueId
...
class NeptuneBackendMock(NeptuneBackend):
        ...
        for op in operations:
            try:
                self._execute_operation(
                    container_id, container_type, op, operation_storage
                )
    ...
    def _execute_operation(
        ...
        visitor = NeptuneBackendMock.NewValueOpVisitor(
            self, op.path, val, operation_storage
        )
    ...
    def get_file_attribute(
        ...
        return FileAttribute(
            name=os.path.basename(val.path)
            if val.file_type is FileType.LOCAL_FILE
            else "",
    ...
    class NewValueOpVisitor(OperationVisitor[Optional[Value]]):
        ...
        def visit_upload_file(self, op: UploadFile) -> Optional[Value]:
            ...
            return File.from_path(
                path=op.get_absolute_path(self._operation_storage), extension=op.ext
            )
        ...
        def visit_upload_file_content(self, op: UploadFileContent) -> Optional[Value]:
            ...
            return File.from_content(
                content=base64_decode(op.file_content), extension=op.ext
            )
